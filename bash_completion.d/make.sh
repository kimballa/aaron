...
# Note: we don't gate this on bash-completion's `have` helper, since that
# function unsets itself once bash-completion finishes its own startup and
# is unavailable by the time our personal completion scripts are sourced.
type make gmake gnumake pmake &>/dev/null &&
_make()
{
